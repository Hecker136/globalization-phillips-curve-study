...
\end{align*}
Where $\pi_t$ is the current inflation, $\pi^e_t$ are the current expectations of inflation. This equation uses the betas as coefficients to measure the line of best fit. Each variable is multiplied by its beta and then the error is added to try to estimate the inflation or inflation gap ($\pi_t-\overline{\pi}$).
\section{Results}
